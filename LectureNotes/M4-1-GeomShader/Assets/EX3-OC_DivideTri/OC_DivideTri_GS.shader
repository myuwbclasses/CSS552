        ...
        Pass
        {
            ...
            void geom(triangle v2g input[3], inout TriangleStream<g2f> triStream)
            {
                ...
                if (ModeIsSet(kComputeNothing)) { // show center point as a small quad
                    ...
                    quad[3].wcPos = mul(unity_ObjectToWorld, center).xyz; // in WC, for lighting calculation in frag shader
                    quad[3].normal = normalize(mul(normal, (float3x3)unity_WorldToObject)); 
                    quad[3].color = (input[0].color + input[1].color + input[2].color) / 3.0;
                        // Note: the cases for color, UV and the normal are WRONG!
                        //       Color/UV: the center may be offseted, so we cannot simply average the UV
                        //       Normal: the normal of the center is not necessarilly the average of the
                        //               deformed triangles
