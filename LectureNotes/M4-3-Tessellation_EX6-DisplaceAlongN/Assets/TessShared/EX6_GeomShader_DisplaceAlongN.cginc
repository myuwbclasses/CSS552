#ifndef EX6_GEOM_SHADER
#define EX6_GEOM_SHADER
...
void Geom(triangle Domain2Geom input[3], inout TriangleStream<Geom2Frag> triStream)
{
    ...
    if (ModeIsSet(kXShowHeight|kZShowHeight)) {
        for (int i = 0; i < 3; i++) {
            ...
            float3 offsetFromX = 0;
            float deltaX = (0.01 * _NormalApproxDist) * _XPerPeriodOC;
            float3 heightDeltaX = input[i].ocTangent; // The height change in the X direction is along the tangent direction
            if (ModeIsSet(kXShowHeight)) {
                ...
                float ydx = ApproxHeightDiff(samplePt, deltaX, kXAxisIsSine) * _XAmplitude * _XZBlend;
                heightDeltaX = 2*deltaX*input[i].ocTangent + ydx * input[i].ocNormal; 
            }

            float3 offsetFromZ = 0;
            float deltaZ = (0.01 * _NormalApproxDist) * _ZPerPeriodOC;
            float3 biNormal = cross(input[i].ocNormal, input[i].ocTangent);
            float3 heightDeltaZ = biNormal; // The height change in the Z direction is along the bi-normal direction;
            if (ModeIsSet(kZShowHeight)) {
                ...
                float ydz = ApproxHeightDiff(samplePt, deltaZ, kZAxisIsSine) * _ZAmplitude * (1-_XZBlend);
                heightDeltaZ = 2*deltaZ*biNormal + ydz * input[i].ocNormal;
            }
            oc += (offsetFromX  + offsetFromZ) * input[i].ocNormal; // Displace along normal direction
            ...
            // Normal
            float3 n = normalize(cross(heightDeltaX, heightDeltaZ)); // The normal is the combination of the height changes in both directions

            if (ModeIsSet(kDebugShowXHeightChange))
                n = normalize(-heightDeltaX);
            else if (ModeIsSet(kDebugShowZHeightChange))
                n = normalize(-heightDeltaZ);
