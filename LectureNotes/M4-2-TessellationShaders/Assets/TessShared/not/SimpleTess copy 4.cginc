...
// V: array of 3 (index 0 to 2) defines worldPos and worldNormal (e.g., from Domain2Frag)
//    V[0] to V[2]: triangle vertices
// BC: barycentric coordinates of OUT, with respect to V[0] to V[2]
//
//    OUT: same data type as V *** vertex position MUST BE computed ***
//
// recompute all normals based on the triangle vertices, and ignore the input normal (e.g., from vertex shader)
//
#define NormalRecompute(V, OUT, BC)     {
    float3 edge1 = V[1].worldPos - V[0].worldPos;
    float3 edge2 = V[3].worldPos - V[0].worldPos;
    float3 n013 = normalize(cross(edge1, edge2));

    edge1 = V[2].worldPos - V[1].worldPos;
    edge2 = V[3].worldPos - V[1].worldPos;
    float3 n123 = normalize(cross(edge1, edge2));

    edge1 = V[0].worldPos - V[2].worldPos;
    edge2 = V[3].worldPos - V[2].worldPos;
    float3 n203 = normalize(cross(edge1, edge2));

    OUT.worldNormal = normalize(BC.x * n123 + BC.y * n203 + BC.z * n013);
    V0
}
