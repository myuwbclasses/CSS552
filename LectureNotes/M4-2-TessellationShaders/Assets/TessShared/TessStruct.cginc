...
struct Vert2Hull {
    ...
    float3 wcPos: TEXCOORD1; // World space position
    float3 ocNormal : NORMAL; //  normal in OC space
    float3 wcNormal: TEXCOORD2; // World space normal
    float2 uv : TEXCOORD3;
    float2 uv1 : TEXCOORD4;
    float4 color : TEXCOORD5; // Vertex color
};
...
struct Domain2Frag {
    ...
    float3 wcPos : TEXCOORD1; // World space position
    float3 ocPos : TEXCOORD0; // Object space position, for convenience
    float3 wcNormal : NORMAL; // World space normal
    float2 uv : TEXCOORD2;
    float2 uv1 : TEXCOORD3;
    float4 color : TEXCOORD4; // Vertex color
};
