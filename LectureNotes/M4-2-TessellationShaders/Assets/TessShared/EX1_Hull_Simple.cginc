...
OutputFromHull SimpleHull(InputPatch<Vert2Hull, 3> points, 
...
{                
    ...
    o.wcPos = points[id].wcPos;

    o.ocNormal = points[id].ocNormal;
    o.wcNormal = points[id].wcNormal;
    ...
    if ((id == 0) && ModeIsSet(kHullNormalOffset)) {
        // For example, we can move the control point along the normal direction
        o.ocPos.xyz += _HullOffsetAmount * o.ocNormal;

        // we have changed the ocPos, then, 
        // we must propagate the change to WC and PC
        o.wcPos = mul(unity_ObjectToWorld, float4(o.ocPos, 1.0)).xyz;
        o.pcPos = UnityObjectToClipPos(float4(o.ocPos, 1.0));

        // we also must re-compite the normal vectors
        // Note: the strengths of Hull shader vs Vert shader, we have access
        //       to all three vertices ... remember, CW ordering, 
        // so we can compute the normal based on the triangle formed by the three vertices
        // Remember, we are working on id=0, so, vertex 0 has changed!
        o.ocNormal = TriangleNormal(o.ocPos, points[1].ocPos, points[2].ocPos);
