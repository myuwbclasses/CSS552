...
struct TriangleHit {
    int hit;
    float dist;  // only valid if hit is 1
    float2 uv;
    float3 bc;
};

struct ClosestHit {
    int hit; // if there is one
    int index;  // this is the RT_ID: 0 (sphere) or 1 (Triangle)
    float3 pt;
    float3 n;
    float2 uv;
};
...
#define bcInTriangle(bc) ((bc.x + bc.y + bc.z) < 1.05)

// compute barycentric 
float3 ComputeBarycentric(float3 pt) {
    float3 bc;
    float3 pv0 = pt - _TheTriangle[0].xyz;
    float3 pv1 = pt - _TheTriangle[1].xyz;
    float3 pv2 = pt - _TheTriangle[2].xyz;
    bc.x = length(cross(pv1, pv2)) * _TriA2Inv;
    bc.y = length(cross(pv0, pv2)) * _TriA2Inv;
    bc.z = length(cross(pv0, pv1)) * _TriA2Inv;
    return bc;
}
