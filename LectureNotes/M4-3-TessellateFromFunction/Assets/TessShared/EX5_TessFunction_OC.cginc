#ifndef EX5_TESS_FUNCTION
#define EX5_TESS_FUNCTION
...
TessFactor ComputeTessFactor(InputPatch<Vert2Hull, 3> points)
{
    ...
    if (ModeIsSet(kApplyOC)) {  // Now followed by OC space 
        // Now use OC peroid as a criterial to decide triange edge length
        // Each edge ... should be no longer than 1/20 of the PerPeriodOC length
        //  
        // Watch out ... this second phase is in OC space (independent from View, not valid?)
        #define OCDiv(i1, i2, TargetEdge)                       \
            edgeDiv = (points[i1].ocPos- points[i2].ocPos).xz;  \
            edgeDiv /= float2(_XPerPeriodOC, _ZPerPeriodOC);    \
            edgeDiv = abs(edgeDiv) * _EdgesPerPeroidOC;         \
            count = max(edgeDiv.x, edgeDiv.y);                  \
            if (count > TargetEdge)                             \
                TargetEdge = count;
